...
filemat = list.files("data/", pattern = "*.csv") # read all files from data-folder into a single list
...
vp_data <- read.csv(paste0("data/",subject),stringsAsFactors = F) %>% #read data from files per subject
  mutate(key_group = key_group) %>%
...
ggplot(vp_data, aes(x = social, fill = condition, y = correct_mean)) +
  ...
  theme_classic()
ggsave(paste0("plot/individuals/",subject,".png"))
...
if(match(subject,filemat) == 1){
  ...
  print(paste0(round(match(subject,filemat)/length(filemat)*100,1), "% ..... ",
               match(subject,filemat), " of ", length(filemat), " files processed! ..... ", 
               round((mean_code_time*length(filemat)-mean_code_time * match(subject,filemat))/60,1), 
               " min remaining"))}
...
vps_summary_long %>% 
  ...
  theme_classic()
ggsave(paste0("plot/ga_correct.png"))
...
vps_summary_long %>% 
  ...
  scale_fill_viridis_d("Condition",end = 0.25,begin = 0.25, guide = "none" ) + 
  theme_classic()
ggsave(paste0("plot/ga_correct_ind.png"))
...
vps_summary_long %>% 
  ...
  scale_fill_viridis_d("Condition",end = 0.25,begin = 0.25, guide = "none" ) + 
  theme_classic()
ggsave(paste0("plot/ga_rt_ind.png"))
