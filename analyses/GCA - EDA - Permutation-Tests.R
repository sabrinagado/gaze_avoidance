...
eda_unified %>%
  ...
  {ggplot(., aes(x=time, y=EDA.mean)) +
      ...
      geom_line(aes(colour=condition)) +
      geom_ribbon(aes(ymin=EDA.mean-EDA.se, ymax=EDA.mean+EDA.se, colour=condition, fill=condition), color = NA, alpha=.2) +
      # geom_segment(data = main_effect_social, aes(x=times_start, xend = times_end, y=-0.2, yend=-0.2, size="Main Effect Social"), colour = "#ff8383", linewidth = 1, inherit.aes=FALSE) +
      # geom_segment(data = main_effect_threat, aes(x=times_start, xend = times_end, y=-0.23, yend=-0.23, size="Main Effect Threat"), colour = "#e874ff", linewidth = 1, inherit.aes=FALSE) +
