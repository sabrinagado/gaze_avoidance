...
if (exists("sample.rate")==F) sample.rate = 100 #sample rate in Hz
if (exists("trials.n")==F) trials.n = .Machine$integer.max #number of trials that shall be analyzed (if more trials, last ones will be taken); Turn off by assigning: .Machine$integer.max
...
# Settings ----------------------------------------------------------------
analysis.range = c(-10, 12) #time in seconds before first and after last marker that shall be analyzed

sample_rate = 1000 
sample_rate_new = 100 #for a priori downsampling
